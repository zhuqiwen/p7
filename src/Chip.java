 ...
 *
 * TD: Complete the Chip(File) constructor.
 */
...
public class Chip {
	...
	public Chip(File file) {
		try {
			...
			for(int i = 0; i < numWires; i++)
			{
				String line = bufferedReader.readLine();
				String tokens[] = line.split(" ");
				wires.add(new Wire(i + 1, new Coord(tokens[0], tokens[1]),  new Coord(tokens[2], tokens[3])));
			}
	...
	/**
	 * Returns true iff the cell at the specified coord is available
	 * for the wire.
	 */
	...
	public static void main(String[] args)
	{
		...
		{
			if(fileList[i].isFile() && fileList[i].getName().startsWith("small"))
			{
